...
class ClassicSlacksTheme extends Themelet {
	...
	protected function build_navigation($page_number, $total_pages, $search_terms) {
		...
		$h_prev = ($page_number <= 1) ? "Prev" : '<a href="'.make_link('/classics/'.$prev).'">Prev</a>';
		$h_next = ($page_number >= $total_pages) ? "Next" : '<a href="'.make_link('/classics/'.$next).'">Next</a>';
	...
	protected function display_page_header(Page $page, $images) {
		...
		$page_title = "Old Timey Slacks Classics";
		if (count($images) > 0) {
	...
	protected function display_page_images(Page $page, $images) {
		$page->add_block(new Block("Classic posters from past Slacks Parties", $this->build_table($images, null), "main", 10, "image-list"));
		$this->display_paginator($page, "classics", null, $this->page_number, $this->total_pages, TRUE);
	}
}
