 ...
 * License: GPLv2
 * Description: Show off classic slacks party posters
...
class ClassicSlacks extends Extension {
    /** @var int */
	private $stpen = 0;  // search term parse event number

	public function onInitExt(InitExtEvent $event) {
		global $config;
		$config->set_default_int("classic_images", 24);
		$config->set_default_bool("classic_tips", true);
		$config->set_default_string("classic_order", "id DESC");
	}

	public function onPageRequest(PageRequestEvent $event) {
		global $database, $page;
		if($event->page_matches("classics")) {
			if(isset($_GET['search'])) {
				// implode(explode()) to resolve aliases and sanitise
				$search = url_escape(Tag::implode(Tag::explode($_GET['search'], false)));
				if(empty($search)) {
					$page->set_mode("redirect");
					$page->set_redirect(make_link("classics/1"));
				}
				else {
					$page->set_mode("redirect");
					$page->set_redirect(make_link('classics/1'));
			...
			try {
				...
				if(SPEED_HAX && $count_search_terms === 0 && ($page_number < 10)) { // extra caching for the first few classics pages
					$images = $database->cache->get("classics:$page_number");
					if(!$images) {
						$images = Image::find_images(($page_number-1)*$page_size, $page_size, $search_terms);
						$database->cache->set("classics:$page_number", $images, 60);
					}
				}
				else {
					$images = Image::find_images(($page_number-1)*$page_size, $page_size, $search_terms);
	...
	public function onSetupBuilding(SetupBuildingEvent $event) {
		$sb = new SetupBlock("Classic Slacks Options");
		$sb->position = 20;

		$sb->add_label("Show ");
		$sb->add_int_option("classic_images");
		$sb->add_label(" images on the classics list");
